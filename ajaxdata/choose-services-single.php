...
$sql = "SELECT *  FROM `ss_services` WHERE `name` LIKE '$filter_name%' AND service_id NOT IN (SELECT parent_id FROM `ss_services`) ORDER BY name ASC LIMIT 0,10";
...
if ($rs->num_rows)
{
	...
	{
		$name = todisplaypath($row['parent_id']);
		$data[] = array(
			'name' => $name.' > '.$row['name'],
			'display_name' => $name.' > '.$row['name'],
