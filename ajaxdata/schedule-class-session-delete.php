...
if($totalcount<$totalhours)
{
	...
	if(isset($_SESSION['ScheduleClass'][$slot_id]) && !empty($_SESSION['ScheduleClass'][$slot_id])) 
	{
		if($quantity==0)
		{
			$message_user = '<div id="rws-formfeedback">Mentoring for '.$_SESSION['ScheduleClass'][$slot_id]["date"].' From '.date("h A", strtotime($_SESSION['ScheduleClass'][$slot_id]["date"].' '.$_SESSION['ScheduleClass'][$slot_id]["start_time"])).' To '.date("h A", strtotime($_SESSION['ScheduleClass'][$slot_id]["date"].' '.$_SESSION['ScheduleClass'][$slot_id]["end_time"])).' has been removed successfully.</div>';
				
			unset($_SESSION['ScheduleClass'][$slot_id]);
				
		}
	}
	else
	{
		$_SESSION['ScheduleClass'][$slot_id]["slot_id"]			= $slot_id;
		$_SESSION['ScheduleClass'][$slot_id]["date"]			= $date_selected;
		$_SESSION['ScheduleClass'][$slot_id]["start_time"]		= $start_time_selected;
		$_SESSION['ScheduleClass'][$slot_id]["end_time"]		= $end_time_selected;
		$_SESSION['ScheduleClass'][$slot_id]["order_id"]		= $order_id;
		$_SESSION['ScheduleClass'][$slot_id]["sp_id"]			= $sp_id;
		$_SESSION['ScheduleClass'][$slot_id]["sp_name"]			= $sp_name;
		
		$message_user = '<div id="rws-formsuccess">Mentoring for '.$_SESSION['ScheduleClass'][$slot_id]["date"].' From '.date("h A", strtotime($_SESSION['ScheduleClass'][$slot_id]["date"].' '.$_SESSION['ScheduleClass'][$slot_id]["start_time"])).' To '.date("h A", strtotime($_SESSION['ScheduleClass'][$slot_id]["date"].' '.$_SESSION['ScheduleClass'][$slot_id]["end_time"])).' has been added successfully. '.$totalcount.' sss '.$totalhours.' sss</div>';
	}
