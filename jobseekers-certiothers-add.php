...
if(isset($_POST["rwsformsubmit"]))
{
	...
	if (empty($_POST['othername']) ) 		{	$errors[]="Please select certificate name field.";		}	
	
	$othername = $_POST["othername"];


		$checkother="select * from addothers where certificate='".$othername."' AND js_id='".$_SESSION["USER"]['ID']."' ";
		$result = $db->query($checkother);
		$otherrows = $result->num_rows;

		if($otherrows>0){

			$errors[]='You have already added the <em>'.$othername.'</em> certificate.'; 		
	...
	if(empty($errors)) 
	{
			...
			if($post_id>0)
			{
				$update_query = "UPDATE `addothers` SET js_id = '".$_SESSION["USER"]["ID"]."', certificate = '$othername', number = '$number', issue_auth = '$issue_auth', issuedate = '$issue_date', expdate = '$expiry_date' WHERE `id`='$post_id' AND `js_id`=".$_SESSION["USER"]['ID'];
			...
			else
			{
				...
				$update_query = "INSERT INTO `addothers` SET js_id = '".$_SESSION["USER"]["ID"]."', certificate = '$othername', number = '$number', issue_auth = '$issue_auth', issuedate = '$issue_date', expdate = '$expiry_date' ";
				$update_result = $db->query($update_query);	
			...
			echo "<script>document.location.href='".$baseurl."jobseekers-certiothers-add.php'</script>";
			exit;
...
if(empty($_POST)) {
	...
	if($post_id>0)
	{
		$select_query = 'SELECT * FROM `addothers` WHERE `id`="'.$post_id.'" ';
...
<div class="rws-maincontentinner">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
		  ...
		  <?php
    		$query="SELECT * FROM addothers WHERE js_id=".$_SESSION["USER"]['ID']." ";
        ...
        <div class="rws-module">
               <!-- <div class="mtitle">Passpost Data <span class="rws-addnewitem"><a href="jobseekers-education-add.php">Add New</a></span></div>   -->
                ...
                <div class="mtitle"> Other Details <span class="rws-addnewitem"></span></div>
        ...
        <form action="" method="post" enctype="multipart/form-data" name="gtregisterforms" id="gtregisterforms">
        	...
        	<div class="rws-module">
			   ...
			   <label class="rws-flabel"><h4>ADD OTHERS CERTIFICATE<span>*</span></h4> </label> 
                		<br>	<br>
					...
					<label class="rws-flabel">Select Certificate<span>*</span></label>		                  	
						...
						$certiother ="select * from certificate";
						$result = $db->query($certiother);
						$rowlist = $result->rows;
						?>

						<select name="othername" id="othername">
			...
			<!--		<input type="button" name="rwsformsubmit" id="rwsformsubmit" value="Back" onClick="document.location.href='jobseekers-certificate.php'" class="rwsbutton width_100" />    -->
					<input type="button" name="rwsformsubmit" id="rwsformsubmit" value="Next" onClick="document.location.href='jobseekers-certistcw-add.php'" class="rwsbutton width_100" />  
