...
<div class="rws-breadcrumbs">
    <div class="container">
        <div class="row">
            <div class="col-xs-12">
                <a href="<?php echo $baseurl;?>master/dashboard.php">Home</a>
...
<div class="rws-maincontentinner">
    ...
    <div class="row">
        ...
        $query="SELECT t1.*, t2.firstname, t2.lastname, t2.email, t2.mobile, t2.profile_pic, t2.resume, t2.location, t2.professional_headline, t3.section, t3.jobtitle, t3.category, t3.job_type, t3.vessel, t3.location as joblocation, t4.firstname as emp_firstname, t4.lastname as emp_lastname, t4.email as emp_email, t4.mobile as emp_mobile, t4.company as emp_company FROM ss_jobseekers_jobapplied as t1 LEFT JOIN ss_jobseekers as t2 ON t1.js_id=t2.id  LEFT JOIN ss_employer_jobs as t3 ON t1.job_id=t3.id LEFT JOIN ss_employer as t4 ON t1.emp_id=t4.id  $conditon ";
			...
			if($orderfield!="") 
			{ 				
				$urltoshow = $baseurl."all_employer-applicant-list.php?page=emp&PageNo=".$pagenum."&field=".$orderfield."&order=".$orderby.$urlconditions;				
				$urltopage = $baseurl."all_employer-applicant-list.php?page=emp".$urlconditions; 				
			}				
			else 				
			{				
				$urltoshow = $baseurl."all_employer-applicant-list.php?page=emp&PageNo=".$pagenum.$urlconditions; 				
				$urltopage = $baseurl."all_employer-applicant-list.php?page=emp".$urlconditions; 				
			}
			...
			if($orderby != "" && $orderby == "ASC")
			{
				...
				$show_name = '<a href="'.$urltopage.'&field=firstname&order=DESC" title="Descending Order.">Name <i class="fa fa-sort" aria-hidden="true"></i></a>';	
				$show_email = '<a href="'.$urltopage.'&field=email&order=DESC" title="Descending Order.">Email <i class="fa fa-sort" aria-hidden="true"></i></a>';	
				$show_jobtitle = '<a href="'.$urltopage.'&field=jobtitle&order=DESC" title="Descending Order.">Rank <i class="fa fa-sort" aria-hidden="true"></i></a>';	
			...
			else
			{
				...
				$show_name = '<a href="'.$urltopage.'&field=firstname&order=ASC" title="Ascending Order.">Name <i class="fa fa-sort" aria-hidden="true"></i></a>';	
				$show_email = '<a href="'.$urltopage.'&field=email&order=ASC" title="Ascending Order.">Email <i class="fa fa-sort" aria-hidden="true"></i></a>';	
				$show_jobtitle = '<a href="'.$urltopage.'&field=jobtitle&order=ASC" title="Ascending Order.">Rank <i class="fa fa-sort" aria-hidden="true"></i></a>';	
                	...
                	<table class="rwsorderitems" cellpadding="0" cellspacing="0" border="0" width="100%">
						...
						?>
                        	<tr <?php echo $gtclasstr;?>>
                                ...
                                <td align="left" valign="top"><a href="<?php echo $baseurl;?>emp-jobseeker-details.php?post_id=<?php echo $row["js_id"]; ?>"><?php echo $row["firstname"].' '.$row["lastname"]; ?></a></td>
                                <td align="left" valign="top"><?php echo $row["jobtitle"]; ?></td>
                                <td align="left" valign="top"><?php echo $row["category"];?></td>
                                <td align="left" valign="top"><?php echo  $row["vessel"]; ?></td>
                                <td align="left" valign="top"><?php echo toshowdatetime($row["apply_date"]); ?></td>
