...
if(isset($_POST["rwsformsubmit"]))
{
	...
	elseif(tocheckoldpwdusers($_POST['oldpassword'], "ss_agent", $_SESSION["AGN"]['ID'])==0) {$errors[]='Please fill out the correct old password field.';}
	...
	if(empty($errors)) 
	{
			...
			$update_query = "UPDATE `ss_agent` SET password = '$password' WHERE `id`=".$_SESSION["AGN"]['ID'];

			$update_result = $db->query($update_query);	
			
			$_SESSION['GTMsgtoUser'] = '<div id="rws-formsuccess">Great! Your password has been updated successfully.</div>';	

			echo "<script>document.location.href='".$baseurl."agent-change-password.php'</script>";
			exit;
	}
	else
	{
