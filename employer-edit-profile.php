<?php include("includes/config.php");  $gtpage = "Employer-Edit-Profile"; $gtjqueryui = "Yes"; $gtckeditor = "Yes";
	...
	if(empty($errors)) 
	{
		...
		$array_rand = array("rad", "dfd","met","axz", "yza", "tst", "tms", "lpg","art","mec","efc","mar","kar","lad","mah");
			...
			*/
			...
			$update_query = "UPDATE `ss_employer` SET firstname = '$firstname', lastname = '$lastname', company = '$company', mobile = '$mobile', logo = '$profilename', coverpic = '$covername', address = '$address', city = '$city', state = '$state', pincode = '$pincode', country = '$country', description = '$description', website = '$website', twitter = '$twitter' , linkedin = '$linkedin' ,
			facebook = '$facebook' , instagram = '$insta'  WHERE `id`=".$_SESSION["EMP"]['ID'];

			$update_result = $db->query($update_query);	
			
			$_SESSION["EMP"]['Firstname']		=	$firstname;
			$_SESSION["EMP"]['Lastname']		=	$lastname;				
			$_SESSION["EMP"]['Mobile']			=	$mobile;	
			$_SESSION["EMP"]['Picture']			=	$profilename;
			
			$js_id = $_SESSION["EMP"]['ID'];
