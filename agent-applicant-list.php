...
<?php include("includes/config.php");  $gtpage = "agent-applicant-list";
checkagentlogin(); 
...
if(isset($_GET['Action']))
{
	...
	{
		case 'Delete':	
			$sql="DELETE FROM `ss_agent_vacancy` WHERE `id`='$post_id' AND `agn_id`=".$_SESSION["AGN"]['ID'];
			$rs = $db->query($sql);
		...
		case 'Shortlist':				
			$sql="UPDATE `ss_agent_vacancy` SET `status`='2', `assigned_date`='$gtcurrenttime' WHERE `id`='$post_id' AND `emp_id`=".$_SESSION["AGN"]['ID'];
			$rs = $db->query($sql);
		...
		case 'Hire':				
			$sql="UPDATE `ss_agent_vacancy` SET `status`='4', `assigned_date`='$gtcurrenttime' WHERE `id`='$post_id' AND `emp_id`=".$_SESSION["AGN"]['ID'];
			$rs = $db->query($sql);
		...
		case 'Reject':				
			$sql="UPDATE `ss_agent_vacancy` SET `status`='3', `assigned_date`='$gtcurrenttime' WHERE `id`='$post_id' AND `emp_id`=".$_SESSION["AGN"]['ID'];
			$rs = $db->query($sql);
			$_SESSION["GTMsgtoUser"] = '<div id="rws-formsuccess">Record has been disabled successfully.</div>';	
			echo "<script>document.location.href='".$_SESSION["RedirectPageURL"]."'</script>";
		exit;	
    ...
    <div class="row">
    	<div class="col-md-3">
         ...
         <?php		 
    		...
    		$query="SELECT t1.*, t2.*, t3.* , t4.firstname as emp_firstname, t4.lastname as emp_lastname, t4.email as emp_email, t4.mobile as emp_mobile, t4.company as emp_company FROM ss_agent_vacancy as t1 LEFT JOIN ss_agent as t2 ON t1.js_id=t2.id  LEFT JOIN ss_agent_vacancy as t3 ON t1.job_id=t3.id LEFT JOIN ss_agent as t4 ON t1.emp_id=t4.id WHERE t1.emp_id=".$_SESSION["AGN"]['ID']." $conditon ";
			...
			if($orderfield!="") 
			{ 				
				$urltoshow = $baseurl."agent-applicant-list.php?page=agn&PageNo=".$pagenum."&field=".$orderfield."&order=".$orderby.$urlconditions;				
				$urltopage = $baseurl."agent-applicant-list.php?page=agn".$urlconditions; 				
			}				
			else 				
			{				
				$urltoshow = $baseurl."agent-applicant-list.php?page=agn&PageNo=".$pagenum.$urlconditions; 				
				$urltopage = $baseurl."agent-applicant-list.php?page=agn".$urlconditions; 				
			}
                	...
                	<table class="rwsorderitems" cellpadding="0" cellspacing="0" border="0" width="100%">
						...
						?>
                        	<tr <?php echo $gtclasstr;?>>
                                ...
                                <td  valign="top"><a href="<?php echo $baseurl;?>emp-jobseeker-details.php?post_id=<?php echo $row["job_id"]; ?>"><?php echo ucwords(strtolower($row["fullname"])); ?></a></td>
