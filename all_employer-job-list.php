...
if(isset($_GET['Action']))
{
	...
	switch($action)
	{
		...
		/*	$sql="SELECT * FROM `ss_employer_jobs` WHERE `id`='$post_id' AND `emp_id`=".$_SESSION["EMP"]['ID'];   */
			$sql="SELECT * FROM `ss_employer_jobs` WHERE status='1' ";
			$rs = $db->query($sql);
...
<div class="rws-maincontentinner">
    ...
    <div class="row">
    	...
    	//	$query="SELECT t1.*, IFNULL(t2.totalapplicants,0) as totalapplicants FROM ss_employer_jobs as t1 $queryjoin WHERE  $conditon ";

		      $query="SELECT * FROM `ss_employer_jobs`  as t1 $queryjoin ";
			...
			if($orderfield!="") 
			{ 				
				$urltoshow = $baseurl."all_employer-job-list.php?page=emp&PageNo=".$pagenum."&field=".$orderfield."&order=".$orderby.$urlconditions;				
				$urltopage = $baseurl."all_employer-job-list.php?page=emp".$urlconditions; 				
			}				
			else 				
			{				
				$urltoshow = $baseurl."all_employer-job-list.php?page=emp&PageNo=".$pagenum.$urlconditions; 				
				$urltopage = $baseurl."all_employer-job-list.php?page=emp".$urlconditions; 				
			}
					...
					//	if($row["section"]=="Shore") { $editlink = "employer-post-shore-jobs.php"; $catarray = $array_category_shore; } //else { $editlink = "employer-post-offshore-jobs.php";  $catarray = $array_category_offshore;}
						...
						?>
                        	<tr <?php echo $gtclasstr;?>>
                                ...
                                <td align="left" valign="top"><a href="<?php echo $baseurl;?>job-details.php?jobid=<?php echo $row["id"]; ?>"><?php echo $row["jobtitle"]; ?></a></td>
                            ...
                            <!--    <td align="left" valign="top"><?php //echo $array_job_type[$row["vessel"]]; ?></td>   -->
                                ...
                                <td align="left" valign="top"><?php echo $row["section"]; ?></td>
                             <!--    <td align="left" valign="top"><?php echo $row["end_date"]; ?></td>  -->
                                <td align="left" valign="top"><a href="<?php echo $baseurl; ?>employer-applicant-list.php?job_id=<?php echo $row["id"]; ?>">Applicants [<?php echo $row["totalapplicants"]; ?>]</a></td>
