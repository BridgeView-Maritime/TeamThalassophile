...
if(isset($_REQUEST["action"]))
{
	...
	{
		case "Inactive":
			$sql="UPDATE `ss_services` SET `status`='0' WHERE `service_id` in ($chkid)";
			$db->query($sql);
			
			$sql="UPDATE `ss_services_package` SET `status`='0' WHERE `service_id` in ($chkid)";
			$db->query($sql);	
		...
		case "Active":
			...
			$sql="UPDATE `ss_services` SET `status`='1'  WHERE `service_id` in ($chkid)";
			$db->query($sql);
		...
		case "Delete":
			...
			$sql="DELETE FROM `ss_services_package` WHERE `service_id` in ($chkid)";
			$db->query($sql);
...
{
	$sql="UPDATE `ss_services` SET `sort_order`='".$_GET["rwsorder".$val]."'  WHERE `service_id` ='$val'";
	$db->query($sql);
...
/* ----- Action Code Ends HERE ----- */
	...
	$query="SELECT t1.service_id, t1.name, t1.status, t1.date_added, t1.sort_order, t1.total_seat, (select t2.name from ss_services as t2 where t2.service_id=t1.parent_id ) as parent_name FROM ss_services as t1 WHERE t1.service_id > 0 ".$nquery;
	...
	if($orderby != "" && $orderby == "ASC")
	{
		...
		$show_location = '<a href="services-list.php?page=gclt&PageNo='.$pagenum.'&field=total_seat&order=DESC&search='.$search_txt.'" title="Click to Sort in desending order.">Total Seats</a>';	
	...
	else
	{
		...
		$show_location = '<a href="services-list.php?page=gclt&PageNo='.$pagenum.'&field=total_seat&order=ASC&search='.$search_txt.'" title="Click to Sort in desending order.">Total Seats</a>';
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1>
                        ...
                        <small>Front-End Category Product List</small>
                    </h1>
                    <ol class="breadcrumb">
										...
										$j=1; foreach($rowlist as $key => $row) { 
										 ...
										 ?>
                                            ...
                                            <tr>
                                                ...
                                                <td><input name="rwsorder<? echo $row['service_id']; ?>" type="text" value="<?php echo $row["sort_order"]; ?>"><input name="rwscid[]" type="hidden" value="<? echo $row['service_id']; ?>"></td>
