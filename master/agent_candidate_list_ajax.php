...
if ($action == "agent_candidate") {
    ...
    $selectAll =   "SELECT ac.id, ac.job_id,ac.fullname,ac.cv_category,ac.rank, ac.agn_id, j.jobtitle,j.currency, j.salary,j.period,a.firstname,a.email, c.country FROM `ss_agent_candidate` as ac join `ss_agent_vacancy` as av on (av.job_id = ac.job_id) join `ss_agent` as a on(ac.agn_id = a.id) join `ss_employer_jobs` as j on (av.job_id = j.id) join `ss_countries` as c on(ac.country = c.id) where av.status = '1' and ac.job_id = '" . $jobid . "' group by ac.id order by ac.id";
    ...
    ";
		...
		foreach ($rowlist as $key => $row) {
			...
			"</td>
            <td valign='top' style='text-align:left ;'>".'<b>'.'Agent Id</b> : ' . $row['agn_id'].'<br>'.'<b>'.'Name</b> : ' . $row['firstname'].'<br>'.'<b>'.'Email</b> : ' . $row['email'].
			"</td>
            <td valign='top'></td>
