...
$shore="SELECT * FROM `shore_rank` ";
$shrank = $db->query($shore);	
$foundnum = $shrank->num_rows;
...
$name=$_POST['name'];
    ...
    $checkshore="select * from shore_rank where rankname='".$name."' ";
    $checksh = $db->query($checkshore);	
    $crows = $checksh->num_rows;

    if($crows>0){

        echo "<script>alert('Rank Already Present');window.location.href='create_shorerank.php';</script>";
      ...
      if(!empty($_GET['edit'])){
          ...
          $update="UPDATE shore_rank set rankname='".$name."' where id='".$_GET['edit']."'";

          $updateshore = $db->query($update);	

      if($updateshore){

          echo "<script>alert('Rank Updated Successfully');window.location.href='create_shorerank.php';</script>";
        ...
        $insert="Insert into shore_rank set rankname='".$name."' ";
        $insertsh = $db->query($insert);	
        


        if($insertsh){

          echo "<script>alert('Rank Added Successfully');window.location.href='create_shorerank.php';</script>";

      }else{

        echo "<script>alert('Something Went Wrong');window.location.href='create_shorerank.php';</script>";
 ...
 if(!empty($_GET['edit'])){
  ...
  $edit="select * FROM shore_rank where id='".$_GET['edit']."' ";
  $editm = $db->query($edit);	
  $eres = $editm->rows;
...
?>
    ...
    <div class="wrapper row-offcanvas row-offcanvas-left">
     ...
     <a href="create_shorerank.php"><button type="submit" class="btn btn-primary" name="submit"><i class="fa fa-plus" aria-hidden="true"></i> Shore Rank </button></a>
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="active">Manage Shore Rank</li>
