...
<?php include('header.php'); $gtpage = 'stcw_certi'; $listjs = 1; 
  
  $certistcw="SELECT * FROM `certificate` ";
  $stcw = $db->query($certistcw);	
  $foundnum = $stcw->num_rows;
  ...
  $name=$_POST['name'];
      ...
      $checkstcw="select * from certificate where certificate='".$name."' ";
      $chstcw = $db->query($checkstcw);	
      $crows = $chstcw->num_rows;
  
      if($crows>0){
  
          echo "<script>alert('Certificate Already Present');window.location.href='stcw_certi.php';</script>";
     ...
     if(!empty($_GET['edit'])){
            ...
            $update="UPDATE certificate set certificate='".$name."' where id='".$_GET['edit']."'";
  
            $updatestcw = $db->query($update);	
  
        if($updatestcw){
  
            echo "<script>alert('Certificate Updated Successfully');window.location.href='stcw_certi.php';</script>";
          ...
          $insert="Insert into certificate set certificate='".$name."' ";
          $insertstcw = $db->query($insert);	
          
          if($insertstcw){
  
            echo "<script>alert('Certificate Added Successfully');window.location.href='stcw_certi.php';</script>";
  
        }else{
  
          echo "<script>alert('Something Went Wrong');window.location.href='stcw_certi.php';</script>";
   ...
   if(!empty($_GET['edit'])){
    ...
    $edit="select * FROM certificate where id='".$_GET['edit']."' ";
    $editstcw = $db->query($edit);	
      ...
      <div class="wrapper row-offcanvas row-offcanvas-left">
       ...
       <a href="crane_certi.php"><button type="submit" class="btn btn-primary" name="submit"><i class="fa fa-plus" aria-hidden="true"></i> Crane Operator License</button></a>
                  ...
                  <ol class="breadcrumb">
                      ...
                      <li class="active">Add STCW Certificates</li>
                       ...
                       <div class="box-body table-responsive" style="margin-left:200px;width:80%;">
                                  ...
                                  $i=1;	$j=1; foreach($rowlist as $key => $row) { 
                                       ...
                                       ?>
                                          ...
                                          post_id="<?php  echo $row["id"]; ?>" 
                                          onClick="" class="btn btn-danger stcwdelete" title="Delete"><i class="fa fa-trash-o fa-fw" aria-hidden="true"></i></a>&nbsp;
  ...
  <script>
          ...
          $(".stcwdelete").click(function(){
  
          var id=$(this).attr('post_id');
        //  alert(id);
          alert('Are You Want to Delete This Record');
  
          var action = "stcwdelete";
          $.ajax({
          method :"POST",
