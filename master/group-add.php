...
if(isset($_POST["rws-submit"]))
{
	...
	$year_id 					= addslashes($postdata["year"]);
	...
	$subtitle 					= addslashes($postdata["subtitle"]);
	...
	// UPLOAD FILE CODE STARTS 		
		...
		$group_catid_insert = implode(',',$_POST["group_catid"]);
		if(trim($post_id)!="")
		{
			$update_query = "UPDATE `groups` SET `name` = '$title', `status` = '$status', `groupuser` = '$groupuser', `college` = '$college', `course` = '$course', `branch` = '$branch', `year` = '$year_id', `description` = '$description', `groupimage` = '".$imageuploadname[1]."', `imgurl` = '$imgurl', `group_catid` = '$group_catid_insert', `college_id` = '$college_id_insert', `showinalumni` = '$showinalumni', `nameinalumnisection` = '$nameinalumnisection', `askbrachname` = '$askbrachname',`extra_field_alumni_news_type_heading` = '$extra_field_alumni_news_type_heading',`subtitle` = '$subtitle'  WHERE `id`= '$post_id'";  
			...
			$update_querycat = $db->query("DELETE FROM `groups_category_relation` WHERE `group_id`='$post_id'");
			foreach($_POST["group_catid"] as $key=>$val)
			{
				$update_querycat = $db->query("INSERT INTO `groups_category_relation` (`group_id`, `group_catid`) VALUES ('$post_id', '$val')");
			}
			$msg_result='<div id="gt-formsuccess">Group has been updated successfully.!</div>';			
		}
		else
		{			
$update_query = "INSERT INTO `groups` (`id`, `name`, `group_catid`, `college_id`, `status`, `groupuser`, `college`, `course`, `branch`, `year`, `description`, `groupimage`, `createdby`, `imgurl`, `created_date`, `showinalumni`, `nameinalumnisection`, `askbrachname`,`extra_field_alumni_news_type_heading`,`subtitle`) VALUES (NULL, '$title', '$group_catid_insert', '$college_id_insert', '$status', '$groupuser', '$college', '$course', '$branch', '$year_id', '$description', '".$imageuploadname[1]."', 'M', '$imgurl', '$gtcurrenttime', '$showinalumni','$nameinalumnisection','$askbrachname','$extra_field_alumni_news_type_heading','$subtitle')";
		$update_result = $db->query($update_query);
			...
			foreach($_POST["group_catid"] as $key=>$val)
			{
				$update_querycat = $db->query("INSERT INTO `groups_category_relation` (`group_id`, `group_catid`) VALUES ('$group_id', '$val')");
			}
...
if(isset($_GET["fid"]))
{
$select_query = 'SELECT * FROM `groups` WHERE id = "'.$_GET["fid"].'"';
...
$_SESSION['myForm']['subtitle'] = stripslashes($row['subtitle']);
                        	...
                        	<div class="box box-primary">
                                ...
                                <!-- form start -->
                                    ...
                                    <div class="box-body">
                                       ...
                                       <div class="form-group row">
                                            <div class="col-md-2"><label for="exampleInputEmail1">Group Subtitle Format<span class="error">*</span></label></div>
                                            <div class="col-md-10"><input type="text" name="subtitle" placeholder="Group Sub Title - format {BRANCHNAME} {STARTYEAR}~{ENDYEAR}" id="subtitle" class="form-control" value="<?php echo $_SESSION['myForm']['subtitle']; ?>"></div>
                                        </div>
                                        ...
                                        <div class="form-group row">
                                            <div class="col-md-2"><label for="exampleInputPassword1">Group Category<span class="error">*</span></label></div>
                                            <div class="col-md-10"><?php echo togetgroupcategorylistmultiple('group_catid', $_SESSION['myForm']['group_catid'], $others=""); ?></div>
                                        </div>
                                        ...
                                        <div class="form-group row">
                                         ...
                                         <div class="form-group row">
                                          ...
                                          <div class="form-group row">
                                            <div class="col-md-2"><label for="exampleInputPassword1">ASK + Branch name</label></div>
                                            <div class="col-md-10"><input type="radio" name="askbrachname" value="1" id="RadioGroup1_0" <?php if($_SESSION['myForm']['askbrachname']=='1') { echo 'checked="checked"'; } ?>  /> Yes &nbsp;&nbsp;&nbsp;&nbsp;
                                                  <input type="radio" name="askbrachname" value="0" id="RadioGroup1_1" <?php if($_SESSION['myForm']['askbrachname']=='0') { echo 'checked="checked"'; } ?>  />  No&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;                                              
                                            </div>
                                        </div> 
                                         <div class="form-group row">
