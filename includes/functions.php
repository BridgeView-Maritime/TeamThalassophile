...
function sendMail($mail_fromname,$mail_fromemail,$mail_subject,$mail_to,$mail_cc,$mail_bcc,$mail_message,$mail_attachment){
...
{
	global $baseurl;
	if (empty($_SESSION["EMP"]['ID']) && empty($_SESSION["USER"]['ID'])) {
	} else {
		echo "<script>document.location.href='$baseurl'</script>";
		exit();
	}
...
function checkagentlogin()
{
	$pagename = $_SERVER["REQUEST_URI"];
	global $baseurl, $gtpage, $db;
	if (!isset($_SESSION["AGN"]['ID']) && $_SESSION["AGN"]['ID'] == "") {
		$_SESSION["GTMsgtoUser"] = '<div id="rws-formfeedback"><p>Seems like your session expired. Please login once again.</p></div>';
		echo "<script>document.location.href='" . $baseurl . "agent-login.php'</script>";
	} else {
		/*$query_login="SELECT * FROM `ss_users` WHERE `user_id`='".$_SESSION['GTUserID']."' ";
		$result = $db->query($query_login);
		$row = $result->row;
		...
		$_SESSION['GTAdminValidate']		=	$row["admin_validate"];
		*/
		if (empty($_SESSION["AGN"]['Mobile'])) {
			if ($gtpage != "Agent-Edit-Profile") {
...
function checkagentloginrole($agn_permission)
{
	$pagename = $_SERVER["REQUEST_URI"];
	global $baseurl, $gtpage, $db;

	if ($_SESSION["AGN"]['SubType'] == "SuperAdmin") {
		
	} else {
		$allowedroles = explode(',', $agn_permission);
		if (in_array($_SESSION["AGN"]['SubType'], $allowedroles)) {
		} else {
			
			$_SESSION["GTMsgtoUser"] = '<div id="rws-formfeedback"><p>Seems like you don\'t have permission to view that page. </p></div>';
			echo "<script>document.location.href='" . $baseurl . "agent-dashboard.php'</script>";
			exit;
		}
	}
}
...
// 	$mailin->addTo($mailto, $mailto)->setFrom($mailby, $mailby2)->setReplyTo($mailby, $mailby2)->setSubject($sub)->setText('')->setHtml($body);
// 	$res = $mailin->send();
...
// 	$mail->SMTPAuth = yes;							
// 	$mail->Username = 'admin@teamthalassophile.com';				
...
{
	$arrayprint = '<select name="' . $name . '" id="' . $name . '" class="form-select wide" ' . $onchange . '>
					<option value="">' . $firstoption . '</option>';
	foreach ($array as $key => $val) {
		if ($selected == $key) {
			$arrayprint .= "<option value='".$val."' selected>$val</option>";
		} else {
			$arrayprint .= "<option value='".$val."'>$val</option>";
		}
	}
	return $arrayprint . '</select>';
}

function todisplaymultiplewithcategory($array, $array2, $name, $firstoption, $selected = "", $onchange = "")
{
	...
	if (!empty($array)) {
		$arrayprint .= "<optgroup label=\SOme mane\"> ";
...
function fetch_category($category)
{
	...
	foreach ($rowlist as $key => $row) {
		...
		if ($category == $row['shiptype']) {
			$arrayprint .= '<option value="' . $row['shiptype'] . '" selected>' . $row['shiptype'] . '</option>';
		} else {
			$arrayprint .= '<option value="' . $row['shiptype'] . '" >' . $row['shiptype'] . '</option>';
		}
	}
	echo $arrayprint;
	// return $static;
...
function fetch_category_tabs($category)
{
	global $db;
	$query = "SELECT category FROM `ss_allrank` GROUP BY category;";
	...
	foreach ($rowlist as $key => $row) {
		...
		if ($category == $row['category']) {
			$arrayprint .= '<li class="active"><a href="' . $row['category'] . '" class="nav-link">' . $row['category'] . '</a></li>';
		} else {
			$arrayprint .= '<li><a href="' . $row['category'] . '" class="nav-link">' . $row['category'] . '</a></li>';
		}
	}
	echo $arrayprint;
	// return $static;
...

{
	...
	if ($period == 0) {
		...
		$year2 	= $year1 - ($period - 1);
	}



	if ($editcatid == 0 || $editcatid == "") {
		$topselect = 'selected';
		$selected = "";
	} else {
		$topselect = '';
...
{
	if ($val == "01") {
		$string .= '<option value="01" selected="selected">01</option>';
	} else {
		$string .= '<option value="01">01</option>';
	}

	if ($val == "02") {
		$string .= '<option value="02" selected="selected">02</option>';
	} else {
		$string .= '<option value="02">02</option>';
	}

	if ($val == "03") {
		$string .= '<option value="03" selected="selected">03</option>';
	} else {
		$string .= '<option value="03">03</option>';
	}

	if ($val == "04") {
		$string .= '<option value="04" selected="selected">04</option>';
	} else {
		$string .= '<option value="04">04</option>';
	}

	if ($val == "05") {
		$string .= '<option value="05" selected="selected">05</option>';
	} else {
		$string .= '<option value="05">05</option>';
	}

	if ($val == "06") {
		$string .= '<option value="06" selected="selected">06</option>';
	} else {
		$string .= '<option value="06">06</option>';
	}

	if ($val == "07") {
		$string .= '<option value="07" selected="selected">07</option>';
	} else {
		$string .= '<option value="07">07</option>';
	}

	if ($val == "08") {
		$string .= '<option value="08" selected="selected">08</option>';
	} else {
		$string .= '<option value="08">08</option>';
	}

	if ($val == "09") {
		$string .= '<option value="09" selected="selected">09</option>';
	} else {
		$string .= '<option value="09">09</option>';
	}

	if ($val == "10") {
		$string .= '<option value="10" selected="selected">10</option>';
	} else {
	...
	if ($val == "11") {
		$string .= '<option value="11" selected="selected">11</option>';
	} else {
		$string .= '<option value="11">11</option>';
	}

	if ($val == "12") {
		$string .= '<option value="12" selected="selected">12</option>';
	} else {
		$string .= '<option value="12">12</option>';
	}

	if ($val == "13") {
		$string .= '<option value="13" selected="selected">13</option>';
	} else {
		$string .= '<option value="13">13</option>';
	}

	if ($val == "14") {
		$string .= '<option value="14" selected="selected">14</option>';
	} else {
		$string .= '<option value="14">14</option>';
	}

	if ($val == "15") {
		$string .= '<option value="15" selected="selected">15</option>';
	} else {
		$string .= '<option value="15">15</option>';
	}

	if ($val == "16") {
		$string .= '<option value="16" selected="selected">16</option>';
	} else {
		$string .= '<option value="16">16</option>';
	}

	if ($val == "17") {
		$string .= '<option value="17" selected="selected">17</option>';
	} else {
		$string .= '<option value="17">17</option>';
	}

	if ($val == "18") {
		$string .= '<option value="18" selected="selected">18</option>';
	} else {
		$string .= '<option value="18">18</option>';
	}

	if ($val == "19") {
		$string .= '<option value="19" selected="selected">19</option>';
	} else {
		$string .= '<option value="19">19</option>';
	}

	if ($val == "20") {
		$string .= '<option value="20" selected="selected">20</option>';
	} else {
		$string .= '<option value="20">20</option>';
	}



	if ($val == "21") {
		$string .= '<option value="21" selected="selected">21</option>';
	} else {
		$string .= '<option value="21">21</option>';
	}

	if ($val == "22") {
		$string .= '<option value="22" selected="selected">22</option>';
	} else {
		$string .= '<option value="22">22</option>';
	}

	if ($val == "23") {
		$string .= '<option value="23" selected="selected">23</option>';
	} else {
		$string .= '<option value="23">23</option>';
	}

	if ($val == "24") {
		$string .= '<option value="24" selected="selected">24</option>';
	} else {
		$string .= '<option value="24">24</option>';
	}

	if ($val == "25") {
		$string .= '<option value="25" selected="selected">25</option>';
	} else {
		$string .= '<option value="25">25</option>';
	}

	if ($val == "26") {
		$string .= '<option value="26" selected="selected">26</option>';
	} else {
		$string .= '<option value="26">26</option>';
	}

	if ($val == "27") {
		$string .= '<option value="27" selected="selected">27</option>';
	} else {
		$string .= '<option value="27">27</option>';
	}

	if ($val == "28") {
		$string .= '<option value="28" selected="selected">28</option>';
	} else {
		$string .= '<option value="28">28</option>';
	}

	if ($val == "29") {
		$string .= '<option value="29" selected="selected">29</option>';
	} else {
		$string .= '<option value="29">29</option>';
	}

	if ($val == "30") {
		$string .= '<option value="30" selected="selected">30</option>';
	} else {
		$string .= '<option value="30">30</option>';
	}

	if ($val == "31") {
		$string .= '<option value="31" selected="selected">31</option>';
	} else {
		$string .= '<option value="31">31</option>';
	}
...
{
	if ($val == "01") {
		$string .= '<option value="01" selected="selected">January</option>';
	} else {
		$string .= '<option value="01">January</option>';
	}

	if ($val == "02") {
		$string .= '<option value="02" selected="selected">February</option>';
	} else {
		$string .= '<option value="02">February</option>';
	}

	if ($val == "03") {
		$string .= '<option value="03" selected="selected">March</option>';
	} else {
		$string .= '<option value="03">March</option>';
	}

	if ($val == "04") {
		$string .= '<option value="04" selected="selected">April</option>';
	} else {
		$string .= '<option value="04">April</option>';
	}

	if ($val == "05") {
		$string .= '<option value="05" selected="selected">May</option>';
	} else {
		$string .= '<option value="05">May</option>';
	}

	if ($val == "06") {
		$string .= '<option value="06" selected="selected">June</option>';
	} else {
		$string .= '<option value="06">June</option>';
	}

	if ($val == "07") {
		$string .= '<option value="07" selected="selected">July</option>';
	} else {
		$string .= '<option value="07">July</option>';
	}

	if ($val == "08") {
		$string .= '<option value="08" selected="selected">August</option>';
	} else {
		$string .= '<option value="08">August</option>';
	}

	if ($val == "09") {
		$string .= '<option value="09" selected="selected">September</option>';
	} else {
		$string .= '<option value="09">September</option>';
	}

	if ($val == "10") {
		$string .= '<option value="10" selected="selected">October</option>';
	} else {
		$string .= '<option value="10">October</option>';
	}

	if ($val == "11") {
		$string .= '<option value="11" selected="selected">November</option>';
	} else {
		$string .= '<option value="11">November</option>';
	}

	if ($val == "12") {
		$string .= '<option value="12" selected="selected">December</option>';
	} else {
...
function getlistofsubservices()
{
	...
	if ($total_rows > 0) {
		...
		foreach ($rowlist as $key => $row) {
			...
			if ($row["discount"] > 0) {
				$discount = '<span class="rwsdiscount">*' . $row["discount"] . '%</span>';
			} else {
				$discount = '';
			}
...
function getlistofbusservices($query)
{
	...
	if ($total_rows > 0) {
		...
		foreach ($rowlist as $key => $row) {
			if ($i % 2 == 0) {
				$classshow = 'rws-evenitem';
			} else {
				$classshow = 'rws-odditem';
			}
...
function getlistofbusservicessearch($query, $jdate, $rdate, $bookingfrom)
{
	...
	if ($bookingfrom == "Admin") {
		$urltosubmit = $baseurl . 'master/search-ticket-session.php';
	} else {
		$urltosubmit = $baseurl . 'create-session-cart.php';
	}
...
function togetuserlistcriteria($usertype)
{
	...
	if ($usertype != "") {
		...
		$query = "SELECT user_id FROM `ss_users` ";
	}
...
function togetlistofcategorylinks($array, $section)
{
	...
	foreach ($array as $key => $val) {
		...
		$num = $val['jobtitle'];
		$jobconditon = " `status`='1' AND `jobtitle`='$num' ";
		$arrayprint .= '<li><a href="' . $baseurl . 'search-result-jobs.php?categorysc[]=' . $key . '&amp;gtsec=' . $section . '" title""><h4>' . $val['jobtitle'] . '</h2><span>' . togetjobcountforall($jobconditon) . ' new job posted</span></a></li>';
		$jobconditon = "";
	}
	return $arrayprint . '</li>';
}

function togetlistofcategorylinks2($queryjobs)
{
	...
	foreach ($rowlist as $key => $row) {
		...
		<h4>Rank :<a href="' . $baseurl . 'job-details.php?jobid=' . $row["id"] . '" title="' . $row["jobtitle"] . '"> ' . $row["jobtitle"] . '</a></h4>
		<h6 style="color:#b1035ff0";><i class="fa fa-building-o" aria-hidden="true"></i>&nbsp Company :Confidential</h6>
		<h6><i class="fa fa-usd" aria-hidden="true"></i> Salary :' . $row["salary"] . '</h6>
		<h6><i class="fas fa-briefcase" aria-hidden="true"></i> Experience :' . $row["experience"] . '</h6>
		<h6><i class="fa fa-calendar" aria-hidden="true"></i> Posted Date :' . date('d-m-Y', strtotime($row["add_date"])) . '</h6>
						
		<b>	<p class="text-muted">Posted in ' . $showcategory . ' <em>(' . $row["section"] . ' Job)</em></p></b>
						...
						<a style="background-color:#b1035ff0;color:white !important;" href="' . $baseurl . 'applyjob.php?jobid=' . $row["id"] . '"  jobid="' . $row["id"] . '" title="' . $row["jobtitle"] . '" class="btn job-browse-btn btn-radius br-light job">
									<b>	Apply Now <b></a>
									</div>
...
function togetlistofcategorylinks1($queryjobs)
{
	global $db, $baseurl, $array_location_all, $array_category_shore, $array_category_offshore, $array_job_type;
...
function togetlistofjobs($queryjobs)
{
	...
	foreach ($rowlist as $key => $row) {
		...
		$stringhtml .= '<div class="col-lg-3 col-md-3 col-sm-3" id="single_job">
								<div class="grid-job-widget">
								
					<!--		<span class="job-type job-type_' . $row["job_type"] . '">' . $array_job_type[$row["job_type"]] . '</span>   -->
									<div class="job-like">
					...
					<!--			<div class="avatar box-80">
								...
								<b>	<p class="text-muted">Posted in ' . $showcategory . ' <em>(' . $row["section"] . ' Job)</em></p></b>
									...
									<div class="job-type-grid">
						<!--  <a style="background-color:#0364b1;" href="' . $baseurl . 'job-details.php?jobid=' . $row["id"] . '" title="' . $row["jobtitle"] . '" class="btn job-browse-btn btn-radius br-light">   -->
...
function togetlistofjobsearch($queryjobs)
{
	...
	foreach ($rowlist as $key => $row) {
	 ...
	 <span><i class="fa fa-calendar" aria-hidden="true"></i> Posted Date :' . date('d-m-Y', strtotime($row["add_date"])) . '
	  &nbsp  <b><span class="text-muted">Posted in ' . $showcategory . ' <em>(' . $row["jobarea"] . ' Job)</em></span></b>    </span>   
		...
		<a style="color:white !important ; background: #1f86ef;
		"; href="' . $baseurl . 'applyjob1.php?jobid=' . $row["id"] . '"  jobid="' . $row["id"] . '" title="' . $row["jobtitle"] . '" class="btn job">
		   Apply Now </a>
			  </div>                                                                                           
...
function togetlistofjobseekers($queryjobs)
{
	...
	foreach ($rowlist as $key => $row) {
		if (!empty($row["profile_pic"])) {
			$imgurl = '<a href="' . $baseurl . 'jobseeker-details.php?js_id=' . $row["id"] . '" title="' . $row["firstname"] . '"><img src="' . $baseurl . $row["profile_pic"] . '" alt="' . $row["firstname"] . '" class="img-responsive img-circle" /></a>';
		} else {
			$imgurl = '<a href="' . $baseurl . 'jobseeker-details.php?js_id=' . $row["id"] . '" title="' . $row["firstname"] . '"><img src="' . $baseurl . 'images/no-pic-com-logo.jpg" class="img-responsive img-circle" alt="' . $row["firstname"] . '" /></a>';
		}
		...
		$stringhtml .= '<div class="candidate-list-layout">
									<div class="cll-wrap">
										<div class="cll-thumb">
											' . $imgurl . '
										</div>
										<div class="cll-caption">
											<h4><a href="' . $baseurl . 'jobseeker-details.php?js_id=' . $row["id"] . '" title="' . $row["firstname"] . '">' . $row["firstname"] . ' ' . $row["lastname"] . '</a> <span><i class="ti-briefcase"></i>' . $row["professional_headline"] . '</span></h4>
											<ul>
												<li><i class="ti-location-pin cl-danger"></i>' . $row["location"] . ', ' . $row["country"] . '</li>
												<li><i class="ti-time cl-success"></i>' . $row["additional_skills"] . '</li>
											</ul>
										</div>
									</div>
									
									<div class="cll-right">
										<a href="' . $baseurl . 'jobseeker-details.php?js_id=' . $row["id"] . '" class="btn theme-btn btn-shortlist"><i class="ti-plus"></i>View Details</a>
									</div>
								</div>';
	}
