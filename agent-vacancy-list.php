...
$gtpage = "Agent-vacancy-List";
checkagentlogin();
checkagentloginrole("Admin,Standard");
...
if (isset($_GET['Action'])) {
  ...
  // {
  // 	// case 'Delete':	
  // 	// 	$sql="DELETE FROM `ss_employer_jobs` WHERE `id`='$post_id' AND `agn_id`=".$_SESSION["AGN"]['ID'];
  // 	// 	$rs = $db->query($sql);
  ...
  // 	// case 'Enable':				
  // 	// 	$sql="UPDATE `ss_employer_jobs` SET `status`='1' WHERE `id`='$post_id' AND `agn_id`=".$_SESSION["AGN"]['ID'];
  // 	// 	$rs = $db->query($sql);
  ...
  <div class="container">
    <div class="row">
      <div class="col-xs-12">
        ...
        <a href="javascript:void(0)">Manage Assigned Vacancy</a>
      </div>
    </div>
  </div>
</div>
...
<div class="rws-maincontentinner">
  <div class="" style="padding:0 30px ;">
    <div class="row">
      ...
      $queryjoin .= " LEFT JOIN (SELECT job_id, COUNT(*) AS totalapplicants FROM ss_agent_candidate GROUP BY job_id ) AS t2 ON j.id = t2.job_id ";
      ...
      $query = "SELECT av.job_id, j.add_date, j.start_date, j.end_date, j.company_name, j.category, j.jobtitle, j.area_of_operation, j.description, j.salary, j.currency, j.period, a.firstname, a.logo, a.approval FROM `ss_agent_vacancy` as av join `ss_employer_jobs` as j  on (av.job_id = j.id) JOIN `ss_agent` as a ON ( a.id = av.agn_id ) $queryjoin where av.status = '1' and a.id = '" . $_SESSION['AGN']['ID'] . "' ";
      ...
      if ($orderfield != "") {
        $final_query = $query . " ORDER BY $orderfield $orderby LIMIT $start, $per_page";
      } else {
        $final_query = $query . " ORDER BY add_date DESC LIMIT $start, $per_page";
      }
      ...
      if ($orderfield != "") {
        $urltoshow = $baseurl . "agent-vacancy-list.php?page=emp&PageNo=" . $pagenum . "&field=" . $orderfield . "&order=" . $orderby . $urlconditions;
        $urltopage = $baseurl . "agent-vacancy-list.php?page=emp" . $urlconditions;
      } else {
        $urltoshow = $baseurl . "agent-vacancy-list.php?page=emp&PageNo=" . $pagenum . $urlconditions;
        $urltopage = $baseurl . "agent-vacancy-list.php?page=emp" . $urlconditions;
      }
      ...
      if ($orderby != "" && $orderby == "ASC") {
        ...
        $date = '<a href="' . $urltopage . '&field=date&order=DESC" title="Descending Order.">Date <i class="fa fa-sort" aria-hidden="true"></i></a>';
        ...
        $show_company = '<a href="' . $urltopage . '&field=company&order=DESC" title="Descending Order.">Company Details <i class="fa fa-sort" aria-hidden="true"></i></a>';
        ...
        $show_area = '<a href="' . $urltopage . '&field=job_type&order=ASC" title="Ascending Order.">Job Area <i class="fa fa-sort" aria-hidden="true"></i></a>';
        $show_salary = '<a href="' . $urltopage . '&field=job_type&order=ASC" title="Ascending Order.">Salary <i class="fa fa-sort" aria-hidden="true"></i></a>';
      ...
      <div class="col-md-9">
        <div class="rws-module">
          ...
          <div class="mtitle">Assigned Vacancy <span class="rws-addnewitem">
              <!-- <a href="employer-post-marine-job.php">Post a Job</a> -->
            </span></div>

          <div class="rws-mcontentlist">
            <?php if (isset($_SESSION["GtThanksMsg"])) {
              echo $_SESSION["GtThanksMsg"];
              unset($_SESSION["GtThanksMsg"]);
            }
            ...
            <div class="orderlist">
                ...
                <table class="rwsorderitems" cellpadding="0" cellspacing="0" border="0" width="100%">
                  <thead>
                    <tr>
                      ...
                      <th align="left" valign="top"><?php echo $date; ?></th>

                      <th align="left" valign="top"><?php echo $show_company; ?></th>

                      <th align="left" valign="top"><?php echo $show_area; ?></th>
                      <th align="left" valign="top"><?php echo $show_salary; ?></th>
                  ...
                  <tbody>
                    ...
                    foreach ($rowlist as $key => $row) {
                      ...
                      if ($row["section"] == "Shore") {
                        $editlink = "employer-post-shore-jobs.php";
                        $catarray = $array_category_shore;
                      } else {
                        $editlink = "employer-post-offshore-jobs.php";
                        $catarray = $array_category_offshore;
                      }

                      if ($row["section"] == "Shore") {
                        $editlink = "employer-post-shore-jobs.php";
                        $catarray = $array_category_shore;
                      } else if ($row["section"] == "offshore") {
                        $editlink = "employer-post-offshore-jobs.php";
                        $catarray = $array_category_offshore;
                      } else {
                        $editlink = "agent-post-candidate.php";
                        $catarray = $array_category_offshore;
                      }
                    ...
                    ?>
                      <tr <?php echo $gtclasstr; ?>>
                        ...
                        <td valign="top"><?php
                                          echo $row["job_id"];
                                          ?></td>
                        <td valign="top"><?php echo  date('d-m-Y H:i:s', strtotime($row["add_date"])); ?></td>

                        <!--    <td align="left" valign="top"><?php //echo $array_job_type[$row["vessel"]]; 
                                                              ?></td>   -->
                        <td valign="top" style="text-align:left ;"><b>Company Name</b> : <?php echo $row["company_name"];  ?><br>
                          <b>Rank</b> : <?php echo $row["category"];  ?><br>
                          <b>Jobtitle</b> : <a href="<?php echo $row["job_id"]; ?>"><?php echo $row["jobtitle"]; ?></a>
                        </td>
                        <td valign="top"><?php echo $row["area_of_operation"]; ?></td>
                        <td valign="top" style="text-align:left ;">
                          <!-- <b>Currency</b> : <?php echo $row["currency"];  ?><br> -->
                          <b>Salary</b> : <?php echo $row["currency"];  ?> <?php echo $row["salary"];  ?><br>

                          <b>Period</b> : <?php echo $row["period"]; ?>
                        </td>
                        ...
                        <td valign="top">
                          ...
                          $candidate_count = "SELECT * FROM `ss_agent_candidate` where job_id = '" . $row["job_id"] . "' AND agn_id = '" . $_SESSION['AGN']['ID'] . "'";
                          ...
                          if ($foundnum > 0) {
                            echo '<a href="javascript:void();" jobid="' . $row["job_id"] . '" agentid="' . $_SESSION['AGN']['ID'] . '" id="applicant" class="btn btn-success" data-toggle="modal" data-target="#exampleModal">Applicants (' . $foundnum . ') </a>';
                          } else {
                            echo '<a href="javascript:void();" jobid="' . $row["job_id"] . '" agentid="' . $_SESSION['AGN']['ID'] . '" id="applicant" class="btn btn-danger" data-toggle="modal" data-target="#exampleModal">Applicants (' . $foundnum . ') </a>';
                          }
                          ?>
                        </td>
                        ...
                        <td valign="top" class="rws-actionbtns">
                          ...
                          <p>
                            <!-- <a href="<?php echo $editlink; ?>?post_id=<?php echo $row["job_id"]; ?>" class="btn btn-primary" title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>&nbsp; -->

                            <a href="<?php echo $editlink; ?>?post_id=<?php echo $row["job_id"]; ?>" class="btn btn-success" title="Add"><i class="fa fa-solid fa-plus" aria-hidden="true"></i>
                            </a>&nbsp;

                            <!-- <a href="javascript:void(0);" post_id="<?php echo $row["id"]; ?>" onClick="" class="btn btn-success active" title="Active"><i class="fa fa-check-square-o" aria-hidden="true"></i></a>

														<a href="javascript:void(0);" post_id="<?php echo $row["id"]; ?>" onClick="" class="btn btn-warning inactive" title="Inactive"><i class="fa fa-ban" aria-hidden="true"></i></a>&nbsp; -->
              ...
              <?php } else {
                echo '<div id="rws-formfeedback">There is no jobs posted by you.</div>';
              } ?>
...
<!-- <script>
    var box = document.getElementById("applicant");
    if ($foundnum > 0) {
        box.className = " btn btn-success";
    } else {
        box.className = " btn btn-danger";
        // box.style.left = "50px";
        // box.style.top = "50px";
    }
</script> -->
