...
if(isset($_POST["rwsformsubmit"]))
{
	...
	if (empty($_POST['stcwname']) ) 		{	$errors[]="Please select certificate name field.";		}	
	
	$stcwname = $_POST["stcwname"];


		$checkstcw="select * from addstcw where certificate='".$stcwname."' AND js_id='".$_SESSION["USER"]['ID']."' ";
		$result = $db->query($checkstcw);
		$stcwrows = $result->num_rows;

		if($stcwrows>0){

			$errors[]='You have already added the <em>'.$stcwname.'</em> certificate.'; 
	...
	if(empty($errors)) 
	{
			...
			if($post_id>0)
			{
				$update_query = "UPDATE `addstcw` SET js_id = '".$_SESSION["USER"]["ID"]."', certificate = '$stcwname', number = '$number', issue_auth = '$issue_auth', issuedate = '$issue_date', expdate = '$expiry_date' WHERE `id`='$post_id' AND `js_id`=".$_SESSION["USER"]['ID'];
			...
			else
			{
				...
				$update_query = "INSERT INTO `addstcw` SET js_id = '".$_SESSION["USER"]["ID"]."', certificate = '$stcwname', number = '$number', issue_auth = '$issue_auth', issuedate = '$issue_date', expdate = '$expiry_date' ";
				$update_result = $db->query($update_query);	
			...
			echo "<script>document.location.href='".$baseurl."jobseekers-certistcw-add.php'</script>";
			exit;
...
if(empty($_POST)) {
	...
	if($post_id>0)
	{
		$select_query = 'SELECT * FROM `addstcw` WHERE `id`="'.$post_id.'" ';
...
<div class="rws-maincontentinner">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
		  ...
		  <?php
    		$query="SELECT * FROM addstcw WHERE js_id=".$_SESSION["USER"]['ID']." ";
        ...
        <div class="rws-module">
               <!-- <div class="mtitle">Passpost Data <span class="rws-addnewitem"><a href="jobseekers-education-add.php">Add New</a></span></div>   -->
                ...
                <div class="mtitle"> STCW Details <span class="rws-addnewitem"></span></div>
        ...
        <form action="" method="post" enctype="multipart/form-data" name="gtregisterforms" id="gtregisterforms">
        	...
        	<div class="rws-module">
			   ...
			   <label class="rws-flabel"><h4>ADD STCW CERTIFICATE<span>*</span></h4> </label> 
                		<br>	<br>
					...
					<label class="rws-flabel">Select Certificate<span>*</span></label>		                  	
						...
						$certistcw ="select * from stcw";
						$result = $db->query($certistcw);
						$rowlist = $result->rows;
						?>

						<select name="stcwname" id="stcwname">
				<!--		<option selected ><?php echo $_SESSION['myForm']['certificate']; ?></option>   -->
							<option selected>Select Certificate	</option>	
						<?php
							foreach($rowlist as $key => $row)
							{
								if($row['stcwname']== $_SESSION['myForm']['certificate']){
                                    echo "<option  value='". $row['stcwname'] ."' selected>" .$row['stcwname'] ."</option>"; 
                                    }else{
								echo "<option  value='". $row['stcwname'] ."'>" .$row['stcwname'] ."</option>"; 
									}
							}	
						?>  
						</select>         
			...
			<!--		<input type="button" name="rwsformsubmit" id="rwsformsubmit" value="Back" onClick="document.location.href='jobseekers-certificate.php'" class="rwsbutton width_100" />    -->
					<input type="button" name="rwsformsubmit" id="rwsformsubmit" value="Next" onClick="document.location.href='jobseekers-nokdetails-add.php'" class="rwsbutton width_100" />  
