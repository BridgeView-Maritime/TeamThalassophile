...
if(isset($_POST["rwsformsubmit"]))
{
	...
	if(empty($errors))
	{
		...
		$query_1 = "SELECT * FROM `ss_employer` WHERE `email`='$rwsusername' AND `password`='$rwspassword'";		
		$findnum_1 = togetnumrows($query_1);
		
		$query_2 = "SELECT t1.*, t2.mobile, t2.logo, t2.country, t2.company, t2.coverpic, t2.email as emp_email, t2.firstname as emp_firstname, t2.lastname as emp_lastname FROM ss_employer_manager as t1 INNER JOIN ss_employer as t2 ON t1.emp_id=t2.id WHERE t1.email='$rwsusername' AND t1.password='$rwspassword'";
		...
		if($numrows>0)
		{
		  ...
		  '.$emailfooter;
			...
			else
			{
				if($findnum_2>0)
				{
					$_SESSION["EMP"]['ID']					=	$row["emp_id"];
					$_SESSION["EMP"]['SubType']				=	$row["role"];
					$_SESSION["EMP"]['MID']					=	$row["id"];
				}
				else
				{
					$_SESSION["EMP"]['ID']					=	$row["id"];
					$_SESSION["EMP"]['SubType']				=	"SuperAdmin";
				}
				
				$_SESSION["EMP"]['Firstname']			=	$row["firstname"];
				$_SESSION["EMP"]['Lastname']			=	$row["lastname"];
				$_SESSION["EMP"]['Email']				=	$row["email"];
				$_SESSION["EMP"]['Mobile']				=	$row["mobile"];
				$_SESSION["EMP"]['Picture']				=	$row["logo"];
				$_SESSION["EMP"]['Country']				=	$row["country"];
				$_SESSION["EMP"]['Company']				=	$row["company"];
			
				$_SESSION["EMP"]['Type']				=	"Employer";
				
				$_SESSION["EMP"]['GTUserProfileID']		=	$row["oauth_uid"];
				$_SESSION["EMP"]['GTUserLoginFrom']		=	$row["oauth_provider"];
				$_SESSION["EMP"]['GTUserphotograph']	=	$row["logo"];
				$_SESSION["EMP"]['GTUsercover']			=	$row["coverpic"];
				...
				if($findnum_2==0)
				{
					$query_update_logintime = "UPDATE `ss_employer` SET `last_login` = '$gtcurrenttime' WHERE `id` =".$row["id"];
				}
				else
				{
...
<div class="rws-userpages">
	<div class="rws-userpagesinner">
		<div class="container"><h1>Employer Login</h1></div>
	</div>
</div>
    ...
    <div class="container">
        <div class="row">
            <div class="col-xs-12">
                ...
                <a href="javascript:void(0)">Employer Login</a>
            </div>
        </div>
    </div>
</div>
                	...
                	<div class="rws-module">
                        ...
                        <div class="rws-mcontent">  
                            ...
                            <form action="" method="post" enctype="multipart/form-data" name="gtregisterforms" id="gtregisterforms">
                                ...
                                <div class="rws-fields">
                                    <div class="row">
                                    	<div class="col-xs-5"><a href="<?php echo $baseurl;?>employer-register.php">Signup</a></div>
                                        <div class="col-xs-7 textright"><a href="<?php echo $baseurl;?>employer-forget-password.php">Forgot Password?</a></div>
                                    </div>
                                </div>                                
                            </form>
                        </div>
