...
if(isset($_POST["rwsformsubmit"]))
{
	...
	if(empty($errors)) 
	{
			...
			if(trim($profile_status)=="Deactivate")
			{
				...
				{
					$update_query = "UPDATE `ss_jobseekers` SET status = '2', deactive_till_date = '$deactive_till_date' WHERE `id`=".$_SESSION["USER"]['ID'];
		
					$update_result = $db->query($update_query);	
					
					$_SESSION['GTMsgtoUser'] = '<div id="rws-formsuccess">Great! Your settings has been updated.</div>';	
		
					echo "<script>document.location.href='".$baseurl."jobseekers-setting-delete-account.php'</script>";
					exit;
			...
			{				
				$update_query = "UPDATE `ss_jobseekers` SET status = '1', deactive_till_date = '' WHERE `id`=".$_SESSION["USER"]['ID'];
			...
			{			
				...
				{
					// Delete JobSeeker Account from ss_jobseekers
					$query_delete_1 = "DELETE FROM `ss_jobseekers` WHERE `id`=".$_SESSION["USER"]['ID'];
					$result_1		= $db->query($query_delete_1);
					
					// Delete JobSeeker Data From from ss_jobseekers_availability
					$query_delete_2 = "DELETE FROM `ss_jobseekers_availability` WHERE `js_id`=".$_SESSION["USER"]['ID'];
					$result_2		= $db->query($query_delete_2);
					
					// Delete JobSeeker Data From from ss_jobseekers_certificates
					$query_delete_2 = "DELETE FROM `ss_jobseekers_certificates` WHERE `js_id`=".$_SESSION["USER"]['ID'];
					$result_2		= $db->query($query_delete_2);
					
					// Delete JobSeeker Data From from ss_jobseekers_education
					$query_delete_2 = "DELETE FROM `ss_jobseekers_education` WHERE `js_id`=".$_SESSION["USER"]['ID'];
					$result_2		= $db->query($query_delete_2);
					
					// Delete JobSeeker Data From from ss_jobseekers_employment
					$query_delete_2 = "DELETE FROM `ss_jobseekers_employment` WHERE `js_id`=".$_SESSION["USER"]['ID'];
					$result_2		= $db->query($query_delete_2);
					
					// Delete JobSeeker Data From from ss_jobseekers_employment_details
					$query_delete_2 = "DELETE FROM `ss_jobseekers_employment_details` WHERE `js_id`=".$_SESSION["USER"]['ID'];
					$result_2		= $db->query($query_delete_2);
					
					// Delete JobSeeker Data From from ss_jobseekers_jobapplied
					$query_delete_2 = "DELETE FROM `ss_jobseekers_jobapplied` WHERE `js_id`=".$_SESSION["USER"]['ID'];
					$result_2		= $db->query($query_delete_2);
					
					// Delete JobSeeker Data From from ss_jobseekers_jobapplied_query
					$query_delete_2 = "DELETE FROM `ss_jobseekers_jobapplied_query` WHERE `js_id`=".$_SESSION["USER"]['ID'];
					$result_2		= $db->query($query_delete_2);
					...
					$subject = "Hello Admin, ".$_SESSION["USER"]['Firstname']." user deleted his account successfully from ".$sitename;
					$body = $emailheader.'
				  <tr>
					...
					Hello Admin,<br /><br />
					'.$_SESSION["USER"]['Firstname'].' Account has been deleted successfully from '.$sitename.'.<br /><br />
				...
				/* Send Email To User */
					$subject = "Hello ".$_SESSION["USER"]['Firstname'].", Your Account has been deleted successfully from ".$sitename;
					$body = $emailheader.'
				  <tr>
					<td style="padding:10px;margin:0;line-height:1px;font-size:1px;font-family:\'Helvetica Neue Light\',Helvetica,Arial,sans-serif;color:#66757f;font-size:14px;font-weight:300;line-height:23px;text-align:left">
				...
				sendmail($_SESSION["USER"]['Email'],$subject,$admin_fromemail,$admin_fromname,$body,$path,$resumefilename);
				
				// Remove Session
				unset($_SESSION["USER"]);
				unset($_SESSION["EMP"]); 
	
					echo "<script>document.location.href='".$baseurl."jobseekers-register.php'</script>";
					exit;	
				}
			}
