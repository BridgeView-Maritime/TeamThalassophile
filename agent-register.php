...
$errors[]="Password must contain at least one uppercase letter (A to Z), one lowercase letter (a to z), and one numeric number (0 to 9)";
	...
	if (empty($_POST["capt_code"])) {
		$errors[] = 'We need you to confirm that you are human and not a robot., Please enter the CAPTCHA.';
	}
	...
	if (empty($errors)) {
		if ($_POST['capt'] != $_POST['capt_code']) {
			...
			if (isUnique("email", $_POST['rwsusername'], "ss_agent")) {
				...
				$query_insert = "INSERT INTO `ss_agent` SET firstname = '$firstname', lastname = '$lastname', mobile = '$mobile', email = '$email', password = '$password', company = '$company', country = '$country', termsofuse = '$termsofuse', status = '1', validate = '0', approval = '0', mstatus = '0', add_date = '$gtcurrenttime'";
		  ...
		  '.$emailfooter; */
				...
				$subject2 = "Hello Admin, New Agent Account has been created by $firstname successfully on " . $sitename;
				$message2 = '<tr>
						...
						<div class="rws-module">
							...
							<div class="rws-mcontent">
								...
								<form action="" method="post" enctype="multipart/form-data" name="gtregisterforms" id="gtregisterforms">
									...
									<p>Already have an Account? <a href="<?php echo $baseurl; ?>agent-login.php">Log In</a></p>
								</form>
							</div>
						</div>
						<!-- Module Ends -->
